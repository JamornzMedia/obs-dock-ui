...
// --- L3 Visitor Counter & Refresh Tracker ---
window.initL3VisitorCounter = () => {
    try {
        if (typeof firebase === 'undefined') {
            console.warn("Firebase SDK not loaded, visitor counter disabled.");
            return;
        }
        ...
        // 1. Session-based Online Presence
        const sessionId = Math.random().toString(36).substring(2, 10);
        const presenceRef = firebase.database().ref('l3_online_users/' + sessionId);
        ...
        // 2. Increment Total Page Views (Refresh + 1)
        const totalViewsRef = firebase.database().ref('l3_total_views');
        totalViewsRef.transaction((currentValue) => {
            return (currentValue || 0) + 1;
        });

        // 3. Listen to Online Users Count
        firebase.database().ref('l3_online_users').on('value', (snapshot) => {
            const count = snapshot.numChildren() || 0;
            const badge = document.getElementById('visitor-count-val');
            if (badge) {
                badge.innerText = count;
            }
        });

        // 4. Listen to Total Views Count
        totalViewsRef.on('value', (snapshot) => {
            const totalViews = snapshot.val() || 0;
            const badge = document.getElementById('total-views-val');
            if (badge) {
                badge.innerText = totalViews;
            }
        });

        console.log("L3 Visitor & Pageview Counter Initialized. Session:", sessionId);
    } catch (err) {
        console.error("L3 Visitor Counter Error:", err);
    }
};

// Automatic execution for L3 Visitor Counter and stale cleanup
if (document.readyState === 'complete') {
    setTimeout(() => {
        if (window.initL3VisitorCounter) window.initL3VisitorCounter();
