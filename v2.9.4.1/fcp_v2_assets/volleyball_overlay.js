...
function renderScoreboard(newState, prevState) {
  const { scoreA, scoreB, setsA, setsB, setHistory, colorA, colorB, nameA, nameB, logoUrl, serve, timeoutActive, font, teamWidth, setLimit } = newState;
  ...
  // Status Area (Timeout / Set Point)
  const isSetPointA = (scoreA >= (setLimit || 25) - 1) && (scoreA >= scoreB + 1);
  const isSetPointB = (scoreB >= (setLimit || 25) - 1) && (scoreB >= scoreA + 1);
